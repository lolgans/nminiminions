...
plugin.apiQueryWrapper = function(publisher) {
	return function(realm, resource, method, specifier, document, options, publishingKey) {
		...
		return Promise(
			function(resolve, reject) {
				// console.log('~~~~~~~ apiQueryWrapper PUBLISHER ~~~~~~~~', publishingKey || app.config.rpcApiCall.rpcRoute);
				// log.debug(publisher.rpc.rpcCall.toString());
				var key = 
					publishingKey 
					|| 
					(typeof app.config.rpcApiCall.rpcRoute === 'string') 
