...
module.exports = function(config) {
	...
	if (typeof config.mqs !== 'object') {
		config.mqs = {	};
		log.warn('you should specify "config.mqs" to as a Object and add your rabbitMQ queue infos to it!');
	}
	...
	if (resourceReceiverIndex > -1) {
		...
		config.mqs.resourceReceiver = {
			name: 'resourceReceiver' ,
			id: 'resourceReceiver_' +  config.id,
			queueName: config.id + '_' + UUID,
			subscriptionRoutingKey: config.id + '_' + UUID,				
			server: {
				host: '127.0.0.1',
	...
	if (resourceAnnouncerIndex > -1) {
		...
		config.mqs.resourceAnnouncer = {
			name: 'resourceAnnouncer',
			id: 'resourceAnnouncer_' +  config.id,
			queueName: config.id + '_' + UUID,
			subscriptionRoutingKey: config.id + '_' + UUID,				
			server: {
				host: '127.0.0.1',
	...
	// --- njsonClient
	var njsonClientIndex = config.plugins.indexOf('njsonClient');
	if (njsonClientIndex > -1) {
		config.plugins.splice(njsonClientIndex, 1);			
		config.plugins.push(
			{
				name: 'njsonClient',
				file: __dirname + '/plugins/njsonClient.js'
			}
		);
	}
	return config;
